...
@end
@implementation ASuzzleBlockItemM
/**
 *  展示3秒的提示
 */
...
-(instancetype)initWithPuzzleModel:(ASuzzleItemModel *)puzzleModel
{
    if (self = [super init]) {
        
        //计算坐标和大小
        self.userInteractionEnabled=YES;
        self.backgroundColor = RGBA(241, 242, 247, 1);
        ...
        self.layer.borderColor = RGBA(201, 231, 252, 1).CGColor;
...
-(void)setPuzzleModel:(ASuzzleItemModel *)puzzleModel
{
    _puzzleModel = puzzleModel;
...
{
    [ASuzzleTool AS_funPuzzleMove:self withDragDirection:NULL];
}
-(void)panTarget:(UIPanGestureRecognizer *) panGesture
{
    int  moveDirect = PuzzleItemCtrlDirectNone;
    [ASuzzleTool AS_funPuzzleMove:self withDragDirection:&moveDirect];
    CGFloat xOffSet=0;
    CGFloat yOffSet=0;
    ...
    else if(panGesture.state==UIGestureRecognizerStateEnded)
    {
        ...
        if (f1||f2) {
            ...
            [ASuzzleTool AS_exchangePuzzleWithBank:self];
            //交换和空格的位置，完成调整
        }
        else
        {
...
{
    UIImage * img = [ASuzzleTool getBackImage];
