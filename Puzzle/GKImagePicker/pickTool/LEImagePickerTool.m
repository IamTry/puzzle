...
- (void)selectImgeWithController:(UIViewController *)controller {
    [GKAlertView showActionSheetInView:controller.view WithTitle:nil cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@[@"Camera",@"Album"] clickAtIndex:^(NSInteger buttonIndex) {
        switch (buttonIndex) {
            case 1:
...
- (void)selectImagesWithController:(UIViewController *)controller completion:(void(^)(NSArray *images))completion {
    [GKAlertView showActionSheetInView:controller.view WithTitle:nil cancelButtonTitle:@"cancel" destructiveButtonTitle:nil otherButtonTitles:@[@"camera",@"album"] clickAtIndex:^(NSInteger buttonIndex) {
        switch (buttonIndex) {
            case 1:
            {
                self->_imageBlock = ^(UIImage *image) {
...
- (void)selectImageFromCameraWithController:(UIViewController *)controller {
    [GKAlertView showActionSheetInView:controller.view WithTitle:nil cancelButtonTitle:@"cancel" destructiveButtonTitle:nil otherButtonTitles:@[@"camera"] clickAtIndex:^(NSInteger buttonIndex) {
        if (buttonIndex != 0) {
            [self imageFromCameraWithController:controller];
        }
    }];
...
- (void)imageFromCameraWithController:(UIViewController *)con {
    if ([self isCameraAvailable] && [self doesCameraSupportTakingPhotos]) {
        if ([self cameraPemission]) {
            ...
            [GKAlertView showAlertViewWithTitle:@"Please allow the app to access your camera" message:@"The app requires your consent to access the camera" cancelButtonTitle:@"cancel" otherButtonTitles:@[@"setting"] clickAtIndex:^(UIAlertView *alertView, NSInteger buttonIndex) {
                if (buttonIndex == 1) {
...
//            UINavigationController *navi = [[UINavigationController alloc] initWithRootViewController:controller];
            ...
            [GKAlertView showAlertViewWithTitle:@"Please allow the application to access your album" message:@"The app needs your consent to access the album" cancelButtonTitle:@"cancel" otherButtonTitles:@[@"setting"] clickAtIndex:^(UIAlertView *alertView, NSInteger buttonIndex) {
                if (buttonIndex == 1) {
                    //                    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"prefs:root=Privacy&path=CAMERA"]];
                    NSURL *url = [NSURL URLWithString:UIApplicationOpenSettingsURLString];
                    if ([[UIApplication sharedApplication] canOpenURL:url]) {
